...
function validateInstalledSkill(destination, bundle) {
  ...
  for (const name of names) {
    ...
    const expected = hash(name.endsWith('.md')
      ? bytes.toString('utf8')
        .replace(/npx[ \t]+--no-install[ \t]+block-runner[ \t]+(?=(?:assemble|convert|validate|fix|author|plugin|proof|context)\b)/g, `npx -y block-runner@${version} `)
        .replace(/block-runner@(?:latest|testing)(?!\s+skill\b)/g, `block-runner@${version}`)
      : bytes);
