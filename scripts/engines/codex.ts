...
import { codexExecArgs, MODEL_WORKDIR } from './harness.js';
...
export async function convert(html: string, _opts?: ConvertOptions): Promise<BlockRunnerReport> {
  ...
  try {
    const out = execFileSync(
      ...
      { cwd: MODEL_WORKDIR, input: PROMPT + html, encoding: 'utf8', maxBuffer: 16 * 1024 * 1024, stdio: ['pipe', 'pipe', 'ignore'], timeout: 240000, killSignal: 'SIGKILL' },
    );
