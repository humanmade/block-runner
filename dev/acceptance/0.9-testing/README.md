...
mkdir -p "$EVIDENCE"
node dev/acceptance/0.9-testing/prepare-inputs.mjs --check
node dev/acceptance/0.9-testing/prepare-inputs.mjs \
...
cp "$CANDIDATE_CHECKOUT/dev/acceptance/0.9-testing/author-input.mjs" "$EVIDENCE/consumer/author-input.mjs"
...
for journey in local-asset-feature responsive-panel-grid; do
  ...
  (
    ...
    node "$EVIDENCE/consumer/author-input.mjs" "$journey" \
      "$EVIDENCE/inputs/$journey/dev/benchmarks/authoring/sources" > analysis.json
