...
describe('standalone consumer release archive', () => {
  it('resolves, clean-installs, builds, and inspects the actual release archive', async () => {
    ...
    const generated = compileRegisteredBlock({
      ...
      structure: [
        {
          block: 'core/group', attributes: { className: 'card' }, children: [
            { block: 'core/paragraph', attributes: { content: 'Packaged native content' } }, { id: 'logo', block: 'core/image', attributes: { alt: 'Logo', className: 'logo' } }
          ]
        }
      ], fields: [], locking: { mode: 'contentOnly' }, pattern: { ready: false, overrides: [] },
      styles: {
        strategy: 'mixed', outcomes: [], fonts: [{ assetId: 'body-font', family: fontFamily, fontWeight: '400', fontDisplay: 'swap' }], rules: [
      ...
      assets: [
        {
          id: 'photo', source: sourceImage, status: 'ready', destination: 'assets/photo.png', sha256: createHash('sha256').update(image).digest('hex')
        }, {
          id: 'logo', source: sourceSvg, status: 'ready', destination: 'assets/logo.svg', sha256: createHash('sha256').update(svg).digest('hex'), uses: [{ node: 'logo', attribute: 'url' }]
        }, {
          id: 'body-font', source: sourceFont, kind: 'font', status: 'ready', destination: 'assets/body.woff2', sha256: createHash('sha256').update(font).digest('hex'), fontLicense: { ownership: 'IBM Corp.', license: 'OFL-1.1', notice: fontNotice }
        }
      ], files: [], warnings: [],
    });
    const plan = await planStandalonePluginOutput(output, {
      name: 'acme/notice', files: Object.fromEntries([...generated.files, ...generated.assets].map((file) => [file.path, file.content]))
    });
    ...
    const addition = compileRegisteredBlock({
      version: 1, generatorVersion: '0.9.0', target: { name: 'acme/second-notice', title: 'Second notice' }, structure: [{ id: 'message', block: 'core/paragraph', attributes: { content: 'An independently registered second block.' } }], fields: [{ id: 'message', node: 'message', attribute: 'content', label: 'Message', mode: 'editable' }], locking: { mode: 'contentOnly' }, styles: { strategy: 'native', outcomes: [] }, pattern: { ready: false, overrides: [] }, assets: [], files: [], warnings: []
    });
    const integration = await planExistingPluginOutput(output, { name: 'acme/second-notice', files: Object.fromEntries(addition.files.map((file) => [file.path, file.content])) });
    expect(integration.mode).toBe('existing');
    await writePluginOutput(integration, {
      authorizedReplacements: integration.touchedFiles.filter((file) => file.operation === 'modify').map((file) => file.path)
    });
