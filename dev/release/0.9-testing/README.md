...
`--manual-review` supplies the saved agent-assisted keyboard/visual review. Its input
and plugin ZIP hashes must match the candidate generated by the release checker; a
mismatch stops the run before WordPress testing. The proof reader validates and
retains the complete review record. Omitting it leaves that review blocked; the
checker never creates or assumes a passing review. It is not a human accessibility
certification or the owner’s visual, editing-feel, or manual-accessibility decision;
those remain required until separately retained in the prepared owner-acceptance
journey.
...
`releaseOk` is the release checker's receipt-backed decision for that exact
candidate. It is not a substitute for an owner-acceptance record: prepared inputs
or an unrun owner session must remain labelled `required` or `blocked`, not passed.
...
be described as a release pass. A passing automated acceptance summary also does
not turn prepared-but-unrun owner acceptance into a completed human session.
