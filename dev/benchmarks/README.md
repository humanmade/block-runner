...
segregated from the committed suite) under `dev/benchmarks/presentation/`:

- **`dev/benchmarks/presentation/review.html`** — per layout: the ideal end state beside *each producer's* render
  (the cross-producer comparison matrix).
- **`dev/benchmarks/presentation/scoreboard.html`** — scores over time, built from the results log.

```sh
npm run bench:record   # same run, but appends one record to dev/benchmarks/results.jsonl
```

`dev/benchmarks/results.jsonl` is the **committed history** — one JSON record per recorded run,
...
Run `npm run bench` (prints the scorecard + writes `dev/benchmarks/presentation/`); `npm run bench:record`
