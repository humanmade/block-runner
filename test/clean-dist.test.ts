...
it('moves only the fixed build directory to recoverable Trash when the trash CLI is unavailable', async () => {
  ...
  expect(path.dirname(destination)).toBe(path.join(homedir(), '.Trash'));
