...
export async function writePluginOutput(
  ...
  try {
    ...
    await assertWrittenFilesMatchPreview(plan);
  } finally {
...
/** Verify that the destination still contains exactly the bytes the preview fingerprint covered. */
async function assertWrittenFilesMatchPreview(plan: PluginOutputPlan): Promise<void> {
  for (const file of plan.touchedFiles) {
    const delivered = await readFile(file.path);
    if (!delivered.equals(file.content)) {
      throw new Error(`Written plugin file does not match the confirmed preview: ${file.path}`);
    }
  }
}
