...
export interface ProofFixture {
  ...
  patternOverrides?: {
    ...
    title: string;
    /** Edits made after insertion and verified after save/reopen. */
    editableFields: readonly ProofEditableField[];
  };
  frontend?: {
...
function missingProofConfiguration(context: ProofGateContext): string | undefined {
  ...
  if (gate === 'pattern_overrides' && (!fixture?.patternOverrides?.title || fixture.patternOverrides.editableFields.length === 0)) {
    return 'Pattern proof requires fixture.patternOverrides.title and a non-empty editableFields fixture.';
  }
...
function createRuntime(
  ...
  const browser = async (mode: 'active' | 'deactivated' = 'active'): Promise<Partial<Record<ProofGateId, ProofGateResult>>> => {
    ...
    try {
      await writeFile(config, JSON.stringify({ fixture: options.fixture, baseUrl: 'http://localhost:8888', mode, publication }), 'utf8');
