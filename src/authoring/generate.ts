 ...
 */
export function emitScss(outcomes: readonly AuthoringStyleOutcome[], rootClass: string, unconstrainedRoot = false): string {
  ...
  const lines = [
    ...
    `${rootClass} {`,
    ...(unconstrainedRoot ? [
      // WordPress constrains unknown custom blocks to the theme content column. This only applies
      // to a source outer canvas with a centered, max-width inner group; its own retained rules
      // still own the component's reading measure.
      '  width: 100%;',
...
/**
 * A source canvas commonly has one outer group and an immediately nested group that centers a
 * max-width layout. WordPress would otherwise constrain the generated wrapper before that source
 * layout can run. Keep this inference deliberately structural: an ordinary constrained group
 * never breaks out of its theme content column.
 */
function hasViewportRoot(plan: AuthoringPlan): boolean {
  if (plan.structure.length !== 1 || plan.structure[0]?.block !== 'core/group') return false;
  return (plan.structure[0].children ?? []).some((child) => child.block === 'core/group'
    && centeredMaxWidthGroup(child, plan.styles.rules ?? []));
}

function centeredMaxWidthGroup(
  node: AuthoringStructureNode,
  rules: NonNullable<AuthoringPlan['styles']['rules']>,
): boolean {
  const classes = new Set(typeof node.attributes?.className === 'string'
    ? node.attributes.className.split(/\s+/).filter(Boolean)
    : []);
  ...
  for (const rule of rules) {
    ...
    if (!subjects?.some((subject) => subject.classes.some(({ decoded }) => classes.has(decoded)))) continue;
    for (const declaration of rule.declarations) {
      if (declaration.property === 'max-width') maxWidth = true;
      if (declaration.property === 'margin-left' && declaration.value === 'auto') leftAuto = true;
      if (declaration.property === 'margin-right' && declaration.value === 'auto') rightAuto = true;
    }
  }
