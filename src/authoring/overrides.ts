...
export function supportedPatternOverrideAttributes(
  ...
  return (SUPPORTED_ATTRIBUTES[block] ?? []).filter((attribute) => !attributes || attribute in attributes);
}
...
/**
 * Enable the native block-level pattern override contract. The name is
...
export function applyPatternOverrides(
  ...
  metadata.name = name;
  metadata.bindings = { __default: { source: PATTERN_OVERRIDE_SOURCE } };
...
export function patternOverrideNameFor(
  ...
  return isPatternOverrideBinding(bindings.__default) || isPatternOverrideBinding(bindings[attribute]) ? name : undefined;
}
