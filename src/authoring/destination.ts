...
export async function writeAuthoringOutput(
  ...
  try {
    ...
    await verifyPublishedTargets(runtime);
...
export async function writeGeneratedRegisteredBlock(
  ...
  const files = [...generated.files, ...generated.assets].map((file) => ({
    ...file, content: Buffer.from(file.content),
  }));
  if (generated.manifest.files.length !== files.length) throw new Error('Generated manifest does not cover every output file.');
  for (const file of files) {
    const entries = generated.manifest.files.filter((entry) => entry.path === file.path);
    ...
    if (entries.length !== 1 || !entry || entry.operation !== file.operation
      ...
      || entry.sourcePlanHash !== generated.sourcePlanHash || entry.templateVersion !== generated.templateVersion) {
...
async function verifyPublishedTargets(runtime: PublicationRuntime): Promise<void> {
  for (const entry of runtime.entries) {
    let current: Buffer | undefined;
    try {
      current = await readRegularFile(entry.target, entry.file.path);
    } catch {
